...
the case of the vector \(\bm G=0\) is special, as it has no intrinsic
...
direction of the wavevector \(\bm k\) of the macroscopic field in the
nonretarded limit \(k\to0\).
...
objects. In this case, on the single complex dielectric function \(\epsilon_a\)
and the single complex dielectric dielectric function
\(\epsilon_b\). Nevertheless, if supplied with a large pdl, with lots of
...
at \(\omega_{\text{sp}}=\omega_{\text{p}}/\sqrt2\), and from the dipolar resonance of MG
theory, while additional peaks appear closer to \(\omega_{\text{sp}}\)
