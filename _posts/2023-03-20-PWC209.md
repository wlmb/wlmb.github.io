...
failed. A solution that seems to work is to map each address to the
merged line number as the input is read line by line. If any address
has already been assigned a line number, that is taken as the merged
line. If not, then use the current line. At the end, reverse the
mapping and print the result. The result fits a two liner

    perl -nE 'chomp; ($n[$.],@a)=split / /; ($m)=grep {defined $_} map {$l{$_}} @a; $m//=$.;
    @l{@a}=($m)x@a; END {push @{$a{$l{$_}}}, $_ for keys %l;say "$n[$_] @{$a{$_}}" for keys %a;}' <<FIN
    A a1@a.com a2@a.com
    B b1@b.com
    A a3@a.com a1@a.com
    FIN
...
The full code is similar:
     ...
     9  while(<>){
    10      chomp;
    11      my ($name, @addresses)=split / /;
    12      next unless $name; # skip empty lines
    13      $names[$INPUT_LINE_NUMBER]=$name;
    14      my ($merged)=grep {defined $_} map {$line_of{$_}} @addresses;
    15      $merged//=$INPUT_LINE_NUMBER; # current line by default
    16      @line_of{@addresses}=($merged) x @addresses;
    17  }
    18  my %accounts;
    19  push @{$accounts{$line_of{$_}}}, $_ for(keys %line_of);
    20  # Sort output by account name and line number, and sort addresses
    21  say "$names[$_] ($_): ", join " ",
    22      sort {$a cmp $b} @{$accounts{$_}}
    23      for sort {$names[$a] cmp $names[$b] || $a <=>$b} keys %accounts;
