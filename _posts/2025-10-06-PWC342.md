...
the result is the null string. I used here that digits come before
...
keeping the score, and choosing the maximum. The result fits a one-liner.
...
by one, as it would be useless to shift a 1 unless there is a 0 to its
right. The corresponding code is somewhat more complex:
    ...
    13      warn("Only 0's and 1's permitted in binary string: $_"), next unless /^(0|1)*$/;
    14      warn("Need at least two digits in string: $_"), next unless length >= 2;
    15      my $left=substr my $right=$_,0,1,"";
    16      my $score=($left==0)+((my $tmp=$right)=~tr/1/0/); # initial score
    17      my @scores=($score);
    18      push @scores, $score+=length($2)-length($1) while $right=~s/^(1*)(0+)(\d)/$3/;
    19      say "$_ -> ", max @scores;
    20  }

Examples:

    ./ch-2a.pl 0011 0000  1111 0101 011101
