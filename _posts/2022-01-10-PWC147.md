...
The full code is the following:
     ...
     7  use PDL;
     8  use PDL::NiceSlice;
     9  use POSIX qw(); # don't import to avoid name collisions with PDL
...
An `and` with the first row removes primes don't yield primes
...
numbers such as 100, 101, 102&#x2026; which would have a 0 if
...
one was slow and the other two were relatively fast and
compact.
...
numbers. Completing aquares, we notice that if *p=n(3n-1)/2*, then
...
*n=(s+1)/6*.
    ...
    14      foreach my $j(2..$N){
    15          my $p=$j*(3*$j-1)/2;
    16          foreach my $k(1..$j-1){
    17              my $q=$k*(3*$k-1)/2;
    18              say("p$j=$p\np$k=$q\np$j-p$k=", $p-$q, "=p", index_of($p-$q),
    19                    "\np$j+p$k=", $p+$q, "=p", index_of($p+$q)),
    20                  last J if pentagonal($q+$p) && pentagonal($p-$q);
    21          }
    22  }
...
Results:
    ...
    Time: 0.719220161437988

So, as expected, 100 is too small and produced no result.

    ./ch-2.pl 500

Results:

    Time: 17.1375360488892
...
Results:
    ...
    Time: 320.163414955139

It took a long time, but it worked! The sum and difference of
the 2167-th and the 1020-th pentagon numbers yield the 2395-th
and the 1912-th pentagon number respectively.


## Third: Remove inner loop

I guess the program can be made much faster if I only do
explicitly the outer loop and use PDL for the inner loop.
    ...
    15  my $p=$n*(3*$n-1)/2;
    16  my $check=pentagonal($p);
    17  for my $i (2..$p->nelem){
    18      my $pi=$p(($i-1));
    19      my $pass=which(pentagonal($pi+$p) & pentagonal($pi-$p));
    20      next unless $pass->nelem;
    21      my $j=$pass((0))+1;
    22      my $pj=$p(($j-1));
    23      my $s=$pi+$pj;
    24      my $d=$pi-$pj;
    25      my ($k, $l)=map {index_of($_)} ($d, $s);
    26      say "p$i=$pi\np$j=$pj\np$i-p$j=$d=p$k\np$i+p$j=$s=p$l";
    27      last;
    28  }
    29  say "Time: ", time()-$start;
    30  sub pentagonal {
    31      my $p=shift;
    32      my $p241=24*$p+1;
    33      my $sp241=$p241->sqrt;
    34      return (($p>0)&($sp241**2==$p241) & ($sp241%6==5));
    35  }
    36  sub index_of {
    37      my $p=24*shift()+1;
    38      my $s=sqrt($p);
...
It worked too and is about 300 times faster than the nested
loop. Notice that I had to manipulate the indices to count the
pentagon up from one and not from zero.
...
I can also get rid of the outer loop using PDL.
    ...
    15  my $p=$n*(3*$n-1)/2;
    16  my $check=pentagonal($p);
    17  my $pass=whichND(pentagonal($p+$p(*1)) & pentagonal($p-$p(*1)));
    18  die "No solution found. Try to increase the largest_index" unless $pass->dim(1)>0;
    19  my $ij=$pass(:,(0))+1;
    20  my ($pi, $pj)=map {$p(($_-1))} (my ($i, $j)=map {$ij(($_))} (0,1));
    21  my ($s, $d)=($pi+$pj, $pi-$pj);
    22  my ($k, $l)=map {index_of($_)} ($d, $s);
    23  say "p$i=$pi\np$j=$pj\np$i-p$j=$d=p$k\np$i+p$j=$s=p$l";
    24  say "Time: ", time()-$start;
    25  sub pentagonal {
    26      my $p=shift;
    27      my $p241=24*$p+1;
    28      my $sp241=$p241->sqrt;
    29      return (($p>0)&($sp241**2==$p241) & ($sp241%6==5));
    30  }
    31  sub index_of {
    32      my $p=24*shift()+1;
    33      my $s=sqrt($p);
...
and the speed increased another 50%.
...
(actually, three-liner):
    ...
    perl -MPDL -MPDL::NiceSlice -E '$N=2500; $n=zeroes(long,$N)->xvals+1;
