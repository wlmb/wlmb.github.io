...
I can shorten the code if I don't care about the explanations
and by using markers at the positions where the string is to
be split, based on the binary bits of the counter. Thus a
shorter version of the code is:
...
five-liner, not quite a one-liner, not quite comprehensible.
    ...
    perl -MList::Util=sum0 -MList::MoreUtils=pairwise -E '
    N:foreach $N(@ARGV){$s=sqrt $N;foreach(0..2**(length($N)-1)-1){
      say("N: $N Out: 1"), next N if sum0(os($N,$_))==$s;}say "N: $N Out: 0";}
      sub os{($n, $c)=@_; @bc=map {$_?"-":""} split "", sprintf "%0".length($n)."b", $c;
        @c=split "", $n;return split "-", join "", pairwise {"$a$b" }@bc, @c}' 81 9801 36
