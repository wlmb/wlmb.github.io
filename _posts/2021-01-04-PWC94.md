...
I guess that the inputs would be much more understandable if the input tree is
coded as a yaml file. To that end, I redo the program above using
...
The trees are built from yaml objects: scalars for leaves, value: array to
represent a node with a value and an array of subtrees, null for empty subtrees.
...
For testing purposes, lets make a yaml file and run the program with
