...
comments: true
excerpt: Invert bit and The travelling salesman
...
`N=15` it would take about 3\*14!/8!=6486480=75 days, and for
...
Results:
    ...
    Steps: 100000
    Cities: 9
    Route: [0 2 8 3 5 4 7 1 6 0]
    Length: 4.26598706547594
...
So it seems that my simplistic simulated annealing is not too
good, as it produced a trip with a length 4.26, quite larger than
the optimum 2.96. To understand the problem I plot the result.
![img](../../../../assets/image/2021-07-12-PWC121/Nine.png)

Part of the difficulty is that for this kind of discrete
problems one can not shorten the *steps* as the *temperature* goes
down, i.e., the change of length
when two cities are interchanged is of the same order of
magnitude at the beginning and at the end of the search. In
the usual Monte Carlo simulations for continuous systems one can take large bold
steps at high temperature and short steps at low
temperatures. Thus, in my search there are large fluctuations
at the start, when the temperature is large, but then the
system is trapped at low temperature in a state with a length
which is not the worst, but far from the best. Curiously,
some of the intermediate states
had a length which was much closer to the optimal one, but
then the simulation jumped out of them. With enough time and patience
one could tune the parameters of the simulation to get a
better convergence to the true minimum. In particular,
realistic simulatons would have to be much larger, with a much
slower decrease in *temperature* to avoid becoming trapped in
the wrong *state*. It might help to remember the best solution
generated during the simulation.
Anyway, I just go ahead and attempt a larger trip.
...
Results:
    ...
    Steps: 100000
    Cities: 20
    Route: [0 10 1 9 18 11 3 17 8 4 16 12 7 15 2 19 5 13 6 14 0]
    Length: 9.65196942326497

![img](../../../../assets/image/2021-07-12-PWC121/Twenty.png)

So the simulation got trapped in a trip whose length is about
half the number of cities, while it reached a smaller one of
length 7 and then jumped out of it.


## Realistic distances

Another problem with the programs above with regards to the
traveling salesman problem is that the random distances are
not realistic at all. I would expect any set of distances
computed with any metric, say, the real geometric distance
given by the Euclidean metric or the distance along the
...
assigned distances. Thus, all steps above have a similar cost, whereas in a
real problem, exchanging some neighboring cities would have a small cost
and exchanging farway cities would have a large cost. Thus, I
rewrite the program above but using randomly
positioned cities and calculating their acutal geometric
distance. After initializing the distance matrix the rest of
...
Example:
    ...
    ./ch-2b.pl 9 100000 1 .02  rem2.txt
...
Results:
    ...
    Steps: 100000
    Cities: 9
    Route: [0 1 3 2 5 7 6 8 4 0]
    Length: 2.66107124662034
...
So this time the result was much closer to the actual
optimum. I plot below the evolution of the simulation.

![img](../../../../assets/image/2021-07-12-PWC121/Nine_E.png)

So in this case the system got trapped, but in a *trip* with
a relatively short length.

I try now the case of 20 cities.

    ./ch-2b.pl 20 100000 1 .04  rem3.txt
...
Results:
    ...
    Steps: 100000
    Cities: 20
    Route: [0 18 14 4 15 16 1 19 5 17 2 13 7 10 12 8 6 3 11 9 0]
    Length: 8.68250815006582

![img](../../../../assets/image/2021-07-12-PWC121/Twenty_E.png)

In summary, with the simulated annealing algorithm developed
above one can search in a few seconds for
solutions to the traveling salesman problem which would
nevertheless would be suboptimal. However, the true optimum
obtained from an exhaustive search would be prohibitive even
for modestly sized problems. There are other search
strategies that may be better for this particular problem.
