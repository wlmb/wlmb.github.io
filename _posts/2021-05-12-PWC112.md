...
logarithm of the gamma function (available in `PDL`), replacing the divisions by
...
dumb. It looks at all `$n` bit numbers at filters out those that don't
have to correct number of ones. Then I interpret each 1 as a single
