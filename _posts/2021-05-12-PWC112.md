...
*bit* I can) and restart all the *bits* to its left to a sub-block of
ones followed by zeroes `11..00`. Thus assuming the leftmost bit is
the less significant, I produce in sequence all `$n` bit numbers with `$k` ones.
