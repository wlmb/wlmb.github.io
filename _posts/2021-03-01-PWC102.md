...
A different approach is to systematically build
allowable values of digits obtained by using modular arithmetic with
succesive powers of 10. I write x=x<sub>n</sub>10<sup>n</sup> (Einstein convention),
y=x<sub>N-n</sub>10<sup>n</sup>, x+y=a<sup>2</sup>, with a=a<sub>n</sub>10<sup>n</sup> and x-y=b<sup>2</sup> with
b=n<sub>n</sub>10<sup>n</sup>. Thus, a<sup>2</sup>=a<sub>n</sub>a<sub>m</sub>10<sup>n+m</sup> and
b<sup>2</sup>=b<sub>n</sub>b<sub>m</sub>10<sup>n+m</sup>. I can rewrite everything modulo some number m,
obtaining (x<sub>n</sub>+x<sub>N-n</sub>)r<sub>n</sub>=(a<sub>n</sub>a<sub>m</sub>+b<sub>n</sub>b<sub>m</sub>)r<sub>n+m</sub> (mod m),
(x<sub>n</sub>-x<sub>N-n</sub>)r<sub>n</sub>=(a<sub>n</sub>a<sub>m</sub>-b<sub>n</sub>b<sub>m</sub>)r<sub>n+m</sub> (mod m), where
r<sub>n</sub> are the residues of 10<sup>n</sup> when divided by m. I guess the best
choices for m ought to be primes, but a simple choice is a power
of 10. For example, if I take m=10, all residues are zero except
r<sub>0</sub>=1. Thus the equations read x<sub>0</sub>+x<sub>N</sub>=a<sub>0</sub><sup>2</sup> (mod m) and
x<sub>0</sub>-x<sub>N</sub>=b<sub>0</sub><sup>2</sup> (mod m) with solutions
x<sub>0</sub>=(a<sub>0</sub><sup>2</sup>+b<sub>0</sub><sup>2</sup>)/2 and x<sub>N</sub>=(a<sub>0</sub><sup>2</sup>-b<sub>0</sub><sup>2</sup>)/2 (mod
m). Division is not well defined, so the solution is not
unique. Notice that a<sub>0</sub> and b<sub>0</sub> should both be odd or both even. So a
strategy, as in the second solution above, would be to obtain all
possible combinations of a<sub>0</sub>, b<sub>0</sub>, x<sub>0</sub> and x<sub>N</sub>. Then, obtain a couple
of new equations by using m=100. From them, add to each combination
above a valid values of a<sub>1</sub>, b<sub>1</sub>, x<sub>1</sub> and x<sub>N-1</sub>. The explore 1000
and higher powers, obtaining possible values for pairs of succesive
digits each time.
    ...
    for my $N(@ARGV){
      ...
      A0:
        for my $a0(0..9){
          B0:
    	for my $b0(0..9){
    	    next B0 unless ($a0+$b0)%2==0;
    	  X0:
    	    for my $x0(map {(($a0**2+$b0**2)/2+$_)%10} (0,5)){
    	      xN:
    		for my $xN(map {(($a0**2-$b0**2)/2+$_)%10} (0,5)){
    		    next xN unless $xN>$x0;
    		  A1:
    		    for my $a1(0..9){
    		      B1:
    			for my $b1(0..9){
    			    my $c=$a0**2+$b0**2+20*($a0*$a1+$b0*$b1)-2*$x0;
    			    next B1 unless $c%20==0;
    			    my $d=$a0**2-$b0**2+20*($a0*$a1-$b0*$b1)-2*$xN;
    			    next B1 unless $c%20==0;
    			    for my $x1(map {($c/20+$_)%10} (0,5)){
    				for my $xN1(map {($d/20+$_)%10} (0,5)){
    				    say "$xN$xN1...$x1$x0";
    				}
    			    }
    		      }
