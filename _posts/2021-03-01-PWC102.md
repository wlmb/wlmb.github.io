...
I make a new attempt. Consider an N-digit numbers x=x<sub>l</sub>x<sub>r</sub> and
y=x'<sub>r</sub>x'<sub>l</sub>, where I assume N is even and the primes indicate
...
This seems to be a big improvement compared to the previous codes. The
time still grows exponentially but with a smaller exponent, and the
code seems cleaner.
